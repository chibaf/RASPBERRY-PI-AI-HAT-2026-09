...
python3 "$HOME/scripts/multimodal_voice_assistant.py" \
  ...
  --max-tokens 90 \
  --record-command "arecord -D plughw:CARD=Device,DEV=0 -f S16_LE -r 16000 -c 1 {audio}" \
  --stt-engine hailo \
  --hailo-stt-model "$HOME/hailo-stt-models/Whisper-Base.hef" \
  --telemetry \
  --telemetry-output "$RESULT_DIR/multimodal_voice_hailo_stt_${MEM_GB}.csv" \
  --telemetry-jsonl "$RESULT_DIR/multimodal_voice_hailo_stt_${MEM_GB}.jsonl" \
